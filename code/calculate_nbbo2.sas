...
/*              Each DATA step internal iteration reads a single record of the TAQ   */
/*              quote data. An IF statement guarantees the uniqueness of the         */
/*              Date-Symbol-Time combination during output, and avoids unnecessary   */
/*              estimation of the NBBO variables by performing calculations only     */
/*              when the last BY group value for Time is read.                       */
...
%macro calculate_nbbo2;
    ...
    data &DataNBBO (keep=Date Symbol Time BestBid BestOffer BestBidSize BestOfferSize
        ...
        /* calculate NBBO after all records during a second are read */
        if last.QuoteTime then do;
